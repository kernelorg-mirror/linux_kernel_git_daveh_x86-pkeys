 ...
 *	mprotect_pkey() with an an unallocated pkey
...
#define SYS_alloc_pkey	 377
#define SYS_free_pkey	 378
#define REG_IP_IDX REG_EIP
#define si_pkey_offset 0x18
#else
#define SYS_mprotect_key 325
#define SYS_alloc_pkey	 326
#define SYS_free_pkey	 327
...
void handler(int signum, siginfo_t* si, void* vucontext)
{
...
#ifdef __i386__
	/*
	 * I can't follow why this is necessary, but oh well.
	 */
	fpregs += 0x70;
#endif
	...
	si_pkey_ptr = (u64 *)(((u8 *)si) + si_pkey_offset);
	dprintf1("si_pkey_ptr: %p\n", si_pkey_ptr);
	dump_mem(si_pkey_ptr - 8, 24);
	si_pkey = *si_pkey_ptr;
	assert(si_pkey < NR_PKEYS);
...
int __mprotect_pkey(void *ptr, size_t size, unsigned long orig_prot, unsigned long pkey)
{
	int sret;

	dprintf1("%s(0x%p, %zx, prot=%lx, pkey=%lx)\n", __func__,
...
int alloc_pkey(void)
{
	int ret = syscall(SYS_alloc_pkey, 0, 0);
	dprintf1("%s() SYS_alloc_pkey(0, 0) ret: %d\n", __func__, ret);
	return ret;
}

int free_pkey(unsigned long pkey)
{
	int ret = syscall(SYS_free_pkey, pkey);
...
int alloc_random_pkey(void)
{
	...
	for (i = 0; i < nr_alloced; i++) {
		...
		if (!alloced_pkeys[i])
			continue;
		free_ret = free_pkey(alloced_pkeys[i]);
		assert(!free_ret);
	}
	return ret;
}
...
int mprotect_pkey(void *ptr, size_t size, unsigned long orig_prot, unsigned long pkey)
{
	...
	while (0) {
		...
		ret = __mprotect_pkey(ptr, size, orig_prot, pkey);
		dprintf1("__mprotect_pkey(%p, %zx, prot=0x%lx, pkey=%ld) ret: %d\n",
